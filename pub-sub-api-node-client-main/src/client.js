...
export default class PubSubApiClient {
    ...
    async #subscribe(subscribeRequest) {
        try {
            ...
            subscription.on('data', (data) => {
                if (data.events) {
                    console.log("subscribe events == %j", data);
                    ...
                    data.events.forEach((event) => {
                        if(!schema || schema.id == "123")
                            schema = this.#schemaChache[event.event.schemaId]; //RP custom work around change
                        let parsedEvent = parseEvent(schema, event);
                        parsedEvent.requestId = event.event.id;
        ...
        return new Promise((resolve, reject) => {
            this.#client.GetTopic({ topicName }, (topicError, response) => {
                if (topicError) {
                    ...
                    if (topicName  && topicName.indexOf("__chn") != -1) { //RP custom work around
                        resolve({
                          id: "123",
                          type: {}
                        });
    ...
    async setChannelMeta (meta) {
        console.log("meta == "+JSON.stringify(meta));
        for (const [key, value] of Object.entries(meta)) {
          console.log(`key is == ${key}: ${JSON.stringify(value)}`);
          this.#schemaChache[key] = value
        }
    }
}
